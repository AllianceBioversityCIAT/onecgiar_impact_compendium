...
async def debug_study_relationships(study_id: int, db: Session = Depends(get_db)):
    ...
    try:
        ...
        impact_areas = [
            {"id": row[0], "name": row[1]} for row in impact_areas_result.fetchall()
        ]
        ...
        initiative_count = db.execute(
            initiative_count_query, {"study_id": study_id}
        ).scalar()

        return {
            "study_id": study_id,
            "impact_areas": {"data": impact_areas, "raw_count": impact_count},
            "initiatives": {"data": initiatives, "raw_count": initiative_count},
        }
