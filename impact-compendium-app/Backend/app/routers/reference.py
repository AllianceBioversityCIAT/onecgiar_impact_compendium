...
def get_reference_data(
    db: Session, table: str, id_field: str = "id", name_field: str = "name"
):
    """Generic function to get reference data."""
    try:
        query = text(
            f"SELECT {id_field}, {name_field} FROM {table} ORDER BY {name_field}"
        )
...
async def get_categories(db: Session = Depends(get_db)):
    ...
    try:
        ...
        query = text(
            "SELECT study_category_id, name FROM categories WHERE is_active = 1 ORDER BY name"
        )
        result = db.execute(query)
...
async def get_impact_areas(db: Session = Depends(get_db)):
    ...
    try:
        query = text(
            "SELECT impact_area_id, name FROM clarisa_impacts_areas ORDER BY name"
        )
        result = db.execute(query)
...
async def get_initiatives(db: Session = Depends(get_db)):
    ...
    try:
        query = text(
            "SELECT initiative_id, code, name FROM clarisa_initiatives ORDER BY code"
        )
        result = db.execute(query)
        data = [
            {"id": row[0], "name": f"{row[1]} - {row[2]}"} for row in result.fetchall()
        ]
        if data:
            return data
    except Exception as e:
    ...
    return [
        ...
        {
            "id": 39,
            "name": "INIT-39 - Accelerating Crop Improvement Through Genome Editing",
        },
        {
            "id": 7,
            "name": "INIT-07 - ActioNs for Innovative climate change Mitigation & Adaptation of Livestock",
        },
    ]
...
async def get_centers(db: Session = Depends(get_db)):
    ...
    try:
        query = text(
            "SELECT center_id, acronym, name FROM clarisa_centers ORDER BY acronym"
        )
        result = db.execute(query)
        data = [
            {"id": row[0], "name": f"{row[1]} - {row[2]}"} for row in result.fetchall()
    ...
    return [
        ...
        {
            "id": 3,
            "name": "ICRISAT - International Crops Research Institute for the Semi-Arid Tropics",
        },
    ]
...
async def get_countries(db: Session = Depends(get_db)):
    ...
    try:
        query = text(
            "SELECT country_id, country_name FROM clarisa_countries ORDER BY country_name"
        )
        result = db.execute(query)
        data = [{"id": row[0], "name": row[1]} for row in result.fetchall()]
        if data:
...
async def get_regions(db: Session = Depends(get_db)):
    ...
    try:
        query = text(
            "SELECT region_id, acronym, region_name FROM clarisa_cgiar_regions ORDER BY acronym"
        )
        result = db.execute(query)
        data = [
            {"id": row[0], "name": f"{row[1]} - {row[2]}"} for row in result.fetchall()
