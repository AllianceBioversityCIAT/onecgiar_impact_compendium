...
class CognitoUserService:
    def __init__(self):
        ...
        if not self.user_pool_id:
            ...
            logger.info(
                f"🔧 Initialized Cognito client with default credentials, region: {self.region}"
            )
            ...
            try:
                ...
                logger.info(
                    f"✅ AWS credentials test PASSED - User Pool: {response['UserPool']['Name']}"
                )
        ...
        try:
            ...
            for user in response.get("Users", []):
                user_data = {
                    ...
                    "last_modified_date": user["UserLastModifiedDate"].isoformat()
    ...
    def update_user(self, username: str, name: str, role: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"username": username, "name": name, "role": role, "updated": True}
    ...
    def _mock_create_group(self, group_name: str, description: str) -> Dict[str, Any]:
        return {"group_name": group_name, "description": description, "created": True}

    def _mock_add_user_to_group(self, username: str, group_name: str) -> Dict[str, Any]:
        return {"username": username, "group_name": group_name, "added": True}

    def _mock_remove_user_from_group(
        self, username: str, group_name: str
    ) -> Dict[str, Any]:
        return {"username": username, "group_name": group_name, "removed": True}
                ...
                groups.append(
                    {
                        ...
                        "CreationDate": group["CreationDate"].isoformat()
                        if "CreationDate" in group
                        else None,
                        "LastModifiedDate": group["LastModifiedDate"].isoformat()
                        if "LastModifiedDate" in group
                        else None,
                    }
                )
    ...
    def delete_group(self, group_name: str) -> None:
        ...
        try:
            self.client.delete_group(GroupName=group_name, UserPoolId=self.user_pool_id)
        except ClientError as e:
