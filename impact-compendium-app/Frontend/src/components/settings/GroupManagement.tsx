...
export const GroupManagement: React.FC = () => {
  ...
  const handleCreateGroup = async () => {
    if (!newGroup.name.trim() || !newGroup.description.trim()) {
      showNotification('error', 'Validation Error', 'Group name and description are required');
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        await fetchGroups();
        showNotification('success', 'Success', `Group "${newGroup.name}" created successfully`);
      } else {
  ...
  const handleDeleteGroup = (groupName: string) => {
    showConfirmDialog(
      ...
      async () => {
        try {
          const headers = await authService.getAuthHeaders();
          const response = await fetch(`${API_BASE}/api/users/groups/${groupName}`, {
            method: 'DELETE',
            headers
          });
          ...
          if (response.ok) {
            await fetchGroups();
            showNotification('success', 'Group Deleted', `Group "${groupName}" has been deleted successfully`);
          } else {
            showNotification('error', 'Delete Failed', 'Failed to delete group. Please try again.');
          }
        } catch (error) {
          showNotification('error', 'Delete Failed', 'An error occurred while deleting the group.');
        }
        hideConfirmDialog();
      }
    );
  ...
  const handleAssignUser = async (username: string, groupName: string) => {
    try {
      ...
      if (response.ok) {
        ...
        showNotification('success', 'User Assigned', `${userDisplay} has been assigned to ${groupName}`);
        await Promise.all([fetchUsers(), fetchGroups()]);
      } else {
        showNotification('error', 'Assignment Failed', 'Failed to assign user to group');
      }
    } catch (error) {
      showNotification('error', 'Assignment Failed', 'Failed to assign user to group');
    }
  ...
  return (
    <div className="p-6">
        ...
        <ul className="divide-y divide-gray-200">
          {groups.map((group) => {
            const groupUsers = users.filter(user => 
              user.groups && user.groups.includes(group.GroupName)
            );
                ...
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-gray-400">
                      Created: {new Date(group.CreationDate).toLocaleDateString()}
                    </p>
                    
                    {/* User badges */}
                    {groupUsers.length > 0 && (
                      <div className="mt-2">
                        <p className="text-xs text-gray-600 font-medium">Users in this group:</p>
                        <div className="flex flex-wrap gap-1 mt-1">
                          {groupUsers.map(user => (
                            <span key={user.username} className="inline-flex items-center px-2 py-1 rounded-full text-xs bg-blue-100 text-blue-800">
      ...
      {showAssignModal && (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg p-6 w-96">
            <h3 className="text-lg font-medium mb-4">Assign Users to {selectedGroup}</h3>
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {users.map((user) => (
                <div key={user.username} className="flex items-center justify-between p-2 border rounded">
                  ...
                  <Button
                    size="sm"
                    onClick={() => handleAssignUser(user.username, selectedGroup)}
                  >
                    Assign
                  </Button>
                </div>
