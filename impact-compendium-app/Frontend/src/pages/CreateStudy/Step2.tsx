...
export const CreateStudyStep2: React.FC = () => {
  ...
  const loadReferenceDataItem = useCallback(
      ...
      const loadStudyData = async () => {
        try {
          ...
          if (response.ok) {
            ...
            const keywordIds =
              studyData.keywords?.map((keyword: any) => keyword.id) || [];
            const initiativeIds =
              studyData.initiatives?.map((initiative: any) => initiative.id) ||
              [];
            const centerIds =
              studyData.centers?.map((center: any) => center.id) || [];
            const countryIds =
              studyData.countries?.map((country: any) => country.id) || [];
            const regionIds =
              studyData.regions?.map((region: any) => region.id) || [];
            const impactAreaIds =
              studyData.impact_areas?.map((area: any) => area.id) || [];
  ...
  const handleDropdownOpen = useCallback(
    (key: string) => {
      if (
        options[key as keyof typeof options].length === 0 &&
  ...
  const getSecondaryImpactAreaOptions = () => {
    const filtered = options.impactAreas.filter(option => {
      const isMatch =
        Number(option.value) === Number(formData.primaryCGIARImpactArea);
      return !isMatch;
    });
    return filtered;
  };
  ...
  const getPrimaryImpactAreaOptions = () => {
    return options.impactAreas.filter(
      option =>
        Number(option.value) !== Number(formData.secondaryCGIARImpactArea)
    );
  };
  ...
  const handleNext = () => {
    ...
    if (!formData.primaryCGIARImpactArea) {
      showNotification(
        'error',
        'Please select a Primary CGIAR Impact Area to continue'
      );
      return;
    }
  ...
  const handleSaveDraft = async () => {
    if (!formData.primaryCGIARImpactArea) {
      showNotification(
        'error',
        'Please select a Primary CGIAR Impact Area before saving draft'
      );
      return;
    }
  ...
  return (
    ...
    >
      ...
      {notification.show && (
        <div
          className={`fixed top-4 right-4 z-50 max-w-sm p-4 rounded-xl shadow-2xl transition-all duration-500 transform ${
            notification.show
            ...
            <div className="flex-1 min-w-0">
              ...
              {notification.type === 'error' && (
                <p className="text-xs mt-1 opacity-90">
                  Please correct this issue to continue
                </p>
              )}
            </div>
            <button
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <MultiSelect
                    ...
                    onChange={values =>
                      handleMultiSelectChange('cropProductType', values)
                    }
                  />
                  <MultiSelect
                    label="Keywords"
                    ...
                    onChange={values =>
                      handleMultiSelectChange('keywords', values)
                    }
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <MultiSelect
                    ...
                    onChange={values =>
                      handleInputChange('contributingInitiatives', values)
                    }
                  ...
                  <MultiSelect
                    ...
                    onChange={values =>
                      handleInputChange('contributingCenters', values)
                    }
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <Select
                    ...
                    required
                    options={
                      options.impactAreas.length === 0
                        ? [{ value: '', label: 'Loading...' }]
                        : getPrimaryImpactAreaOptions()
                    }
                  ...
                  <Select
                    ...
                    className={
                      !formData.primaryCGIARImpactArea
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <MultiSelect
                    ...
                    onChange={values =>
                      handleInputChange('countryOfStudy', values)
                    }
                  ...
                  <MultiSelect
                    ...
                    onChange={values =>
                      handleInputChange('cgiarRegions', values)
                    }
