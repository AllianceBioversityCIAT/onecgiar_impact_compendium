...
export const StudyDetailsPanel: React.FC<StudyDetailsPanelProps> = ({
  isOpen,
  onClose,
  studyId,
  onStudyDeleted,
  ...
  useEffect(() => {
    if (isOpen && studyId) {
      ...
      fetch(
        `${import.meta.env.VITE_API_BASE_URL || 'http://localhost:8000'}/api/studies/${studyId}`
      )
        ...
        .then(data => {
          if (data.success && data.data) {
            ...
            setStudy({
              ...
              category:
                typeof studyData.category === 'object'
                  ? studyData.category.name
                  : studyData.category || 'Other',
              year: studyData.year,
              contributors:
                allContributors.length > 0 ? allContributors : 'N/A',
              doi: studyData.doi,
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setNotification({
        ...
        title: 'Delete Failed',
        message:
          'Failed to delete the study. Please try again or contact support if the problem persists.',
      });
    } finally {
      setDeleting(false);
          ...
          <div className="space-y-8 pb-40">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
              ...
              <p className="text-gray-700 leading-relaxed text-base">
                {study.summary || 'No summary available for this study.'}
              </p>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
              ...
              <div className="flex flex-wrap gap-2">
                {Array.isArray(study.contributors) ? (
                  study.contributors.map((contributor, index) => (
                    <span
                      key={index}
                ...
                ) : typeof study.contributors === 'string' &&
                  study.contributors !== 'N/A' ? (
              ...
              <div className="flex flex-wrap gap-2">
                {study.impact_areas && study.impact_areas.length > 0 ? (
                  study.impact_areas.map((area: any, index: number) => (
                    <span
                      key={index}
                      className="inline-flex items-center px-3 py-2 rounded-lg text-sm font-medium bg-green-50 text-green-800 border border-green-200 hover:bg-green-100 transition-colors"
                    >
                      {area.name || area}
                    </span>
                  ))
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                ...
                <div className="flex flex-wrap gap-2">
                  {study.countries.map((country: any, index: number) => (
                    <span
                      key={index}
                      className="inline-flex items-center px-3 py-2 rounded-lg text-sm font-medium bg-orange-50 text-orange-800 border border-orange-200 hover:bg-orange-100 transition-colors"
                    >
                  ...
                  {study.indicators.map((indicator: any, index: number) => (
                    ...
                    >
                      <div className="font-semibold text-gray-900 mb-2">
                        {indicator.measure || indicator.indicator_name}
                      </div>
                      <div className="text-gray-700 flex items-center gap-2">
                        ...
                        <span>
                          {indicator.result_reported ||
                            indicator.indicator_value}
                        </span>
                        {indicator.unit &&
                          !indicator.result_reported?.includes(
                            indicator.unit
                          ) && (
              ...
              <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
                ...
                <div className="space-y-4">
                  {study.narratives.map((narrative: any, index: number) => (
                    <div
                      key={index}
                      className="border-l-4 border-[#FEC750] bg-amber-50/50 pl-4 pr-4 py-3 rounded-r-lg"
                    >
                ...
                <div className="mb-8">
                  ...
                  <div className="mt-3 p-3 bg-red-50 border border-red-200 rounded-lg">
                    <p className="text-sm text-red-800">
                      <span className="font-medium">Warning:</span> All
                      associated data including indicators, relationships, and
                      metadata will be permanently removed from the system.
                    </p>
                  </div>
                </div>
